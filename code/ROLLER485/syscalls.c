/**
 * Minimal newlib syscall stubs for a bare-metal (no-OS) STM32 image.
 * Restored to enable the GCC/CMake build (the Keil project did not need them).
 */
#include <sys/stat.h>
#include <stdlib.h>
#include <errno.h>
...
__attribute__((weak)) int _read(int file, char *ptr, int len)
{
  ...
  for (DataIdx = 0; DataIdx < len; DataIdx++) {
    *ptr++ = __io_getchar ? __io_getchar() : 0;
  }
  return len;
}
...
__attribute__((weak)) int _write(int file, char *ptr, int len)
{
  ...
  int DataIdx;
  for (DataIdx = 0; DataIdx < len; DataIdx++) {
    if (__io_putchar) __io_putchar(*ptr);
    ptr++;
  }
  return len;
}
